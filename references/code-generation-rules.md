...
33. **Refund requires `refundNotifyUrl` for its webhook**: the refund request (`order().refund()`) takes its OWN `refundNotifyUrl`; the `REFUND_NOTIFICATION` webhook does **NOT** fall back to the original order's `notifyUrl`. Omit `refundNotifyUrl` and Waffo emits no refund webhook at all — the refund still succeeds, so this fails silently and is easy to misdiagnose as a Waffo-side problem. Always pass `refundNotifyUrl` on refund. `refundReason` is also required by the API. (Subscription create/change likewise need their own `notifyUrl`.)
