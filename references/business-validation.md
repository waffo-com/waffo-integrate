...
After Step 7 completes, generate a formal acceptance report with this structure:

```
Integration Acceptance Report
├── Overview
│   ├── Project name, date, integration scope
│   ├── Contracted pay methods, MID
│   └── SDK version, environment (Sandbox/Production)
├── Business Scenario Verification
│   ├── Payment flow: order create → payment → webhook → business execution → status update
│   ├── Refund flow: initiate → webhook → revoke benefits → status update
│   ├── Subscription flow: create → activate → renewal → cancel → benefit handling
│   └── Expected vs actual result for each scenario
├── Parameter Check
│   ├── orderDescription — has specific description?
│   ├── goodsName / goodsUrl or appName
│   ├── userEmail format
│   ├── userTerminal matches actual terminal
│   └── Time field format (ISO 8601 UTC+0)
├── Data Integrity Check
│   ├── Idempotency key persisted before API call
│   ├── acquiringOrderID stored
│   ├── refundRequestId returned and persisted
│   └── Redirect URL completeness (success + failed + cancel)
├── Risk Items
│   ├── WaffoUnknownStatusError handling
│   ├── Webhook duplicate push protection
│   ├── Concurrency safety (lock mechanism)
│   └── Amount precision
├── Pay Method Coverage
│   ├── Card-based: per-brand test results
│   ├── Non-card: Sandbox simulation or SKIP
│   └── AP/GP: remind integrator to test manually
├── Passive Verification (Code Review)
│   ├── Payment passive items (8): COVERED / MISSING / PARTIAL
│   └── Subscription passive items (7): COVERED / MISSING / PARTIAL
├── Uncovered Scenarios & Suggestions
├── Next Actions (Pre-Go-Live TODOs)
│   ├── If WeChat Pay: provide production domain to Waffo for channel registration
│   ├── If Google Pay: apply for API access before production
│   ├── If Apple Pay + iframe: Apple Pay cannot be used inside iframe
│   ├── Merchant timeout recommendation: ≥ 8s (minimum), 15s (recommended)
│   ├── DNS cache time recommendation: 60s (Waffo multi-gateway disaster recovery needs)
│   └── If production server is in mainland China: latency risk, recommend SDWAN/VPN/dedicated line
└── Summary & Go-Live Recommendation
```
