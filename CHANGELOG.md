...
- **PHP support** — `references/php.md` templates for the published `waffo/waffo-php` Composer package (SDK init + `Waffo::fromEnvironment()`, order/refund/subscription/config services, framework-agnostic + Laravel + Symfony webhook handlers, and a Sandbox integration test), driven end to end by the skill's 8-step flow. `SKILL.md` gains PHP across Step 1 detection (`composer.json`), Step 2 naming notes, Step 3 frameworks, Step 5 install (`composer require waffo/waffo-php`), and the Reference Loading Map.
- **PHP template compile gate** — `tests/harness/php.mjs` runs `php -l` + PHPStan (level 5) against the pinned published SDK; wired into `tests/compile-templates.mjs` and both CI workflows (`shivammathur/setup-php`), so the release gate now compiles all **five** languages. `static-spec.mjs` and `extract.mjs` extend to PHP.
- **`docs/adding-a-language.md`** — a mechanical checklist for adding the next SDK language.
- **Report renderer** (`waffo-verify . --emit report`) — the acceptance report is now a deterministic projection of an already-validated `.waffo/integration-manifest.json` instead of a hand-typed re-rendering. Rendering is gated exactly like saving; `Skill Compliance Review` is computed from what the gate proved.
- **`schemaVersion` 2** — carries what rendering needs (a `report` block, `tests[].details`, `payMethodCoverage[].country`/`.type`, and quality-finding text). Validated at the report gate; `schemaVersion` 1 keeps gate-only behaviour so in-flight integrations are not blocked.
- **Hook byte-compare gate** — for `schemaVersion` 2 the Claude hook compares the written report against `--emit report` byte for byte, refuses `Edit` of a rendered report, and rejects a file name whose date contradicts `report.date`.
- **`report.notes`** — a first-class home for verification topology, stubbed upstreams and environment caveats.
- Evals 31–35: report-drift guards (compose-by-hand, invented `PASS*` status, dropped `Non-PASS` identifier columns, merged pay-method rows, softened compliance criterion). Eval 36: PHP Laravel payment + refund + webhook.

### Changed

- **`skillVersion` is verified, not self-declared.** The validator reads its own `package.json` and compares strictly with `manifest.skillVersion`; a mismatch is an ERROR that blocks the report gate. The gate/scan output names the running version.
- `SKILL.md`, `references/acceptance-criteria.md`, `docs/enforcement.md` and `README.md` state the rendering contract: on `schemaVersion` 2 the report is emitted, not written; dropping columns, inventing status words, merging pay-method rows and trimming fixed checklists are rejected.

### Fixed

- **Webhook handler-registration scan now recognizes PHP.** `bin/waffo-verify.js` matched only the `.onPayment(` (dot) member-access operator, so PHP `$waffo->webhook()->onPayment(...)` (arrow) was invisible — every PHP integration would have been falsely blocked at the report gate. Patterns are now derived generically over the `.` / `->` / `::` operators and the camelCase/snake_case/PascalCase spellings. Guarded by a new regression test.
- **Refund templates now send `refundNotifyUrl` (Node/Java/Python; Go/PHP already did).** `REFUND_NOTIFICATION` does NOT fall back to the order's `notifyUrl`, so omitting it meant Waffo never sent the refund webhook while the refund itself succeeded — a silent failure. Added `code-generation-rules.md` Rule 33 and a `static-spec.mjs` guard across all five languages; `refundReason` is now sent unconditionally (API-required).
- **Step 6 webhook verification discipline** (`references/integration-verification.md`): subscription/refund notifications are delivered asynchronously with delay + retries, so a short wait is not evidence of non-delivery; keep the receiver alive; confirm the sender side in Datadog before concluding "not delivered"; a missing per-request notify URL is the most common cause; captured Waffo-signed payloads can be replayed to verify handlers offline.
- Closed the gap between "the gate validated the data" and "the delivered report reflects it": an audit of a real merchant report found eight drifts in the hand-typed Markdown, six now structurally impossible (invented `PASS*` status, stripped `Non-PASS` identifier columns, contracted-method rows merged with a method disappearing, etc.).
