...
- **Order-create templates now send `goodsInfo` and all three redirect URLs** (Node/Java/Go/Python). Every create-order template previously omitted `goodsInfo` (risk-control identity — Rule 21) and sent only `successRedirectUrl`, dropping `failedRedirectUrl`/`cancelRedirectUrl` (Rule 19) — contradicting the skill's own README, Rule 21/19, and verification check C6.
...
- **Template compile harness** (`tests/compile-templates.mjs` + `tests/harness/*`) — extracts each language's `references/*.md` code blocks and compiles them against the pinned Waffo SDKs (Java `javac`, Node `tsc`, Go `go build`, Python `py_compile`; optional `pyright`), plus deterministic contract assertions. Wired into `npm test` (`test:unit` / `test:templates` split) and a new CI workflow (`.github/workflows/ci.yml`) that runs all four in `--strict` mode. This mechanically catches template↔SDK drift (phantom fields, wrong methods, unhandled checked exceptions, missing required fields) — the class of defect that previously shipped undetected because nothing compiled the templates.
